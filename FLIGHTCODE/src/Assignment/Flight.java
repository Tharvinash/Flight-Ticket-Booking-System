...
public class Flight<T> {
    ...
    private Ticket[] confirmedList;
    private Queue waitingList;
    private final static int CAPACITY = 5;
    Flight next;

    Flight(String no, String date) {
        this.flightNo = no;
    ...
    public boolean isFull() {
        return this.getAvailableSeat() > CAPACITY;
    }
            ...
            for (int i = 0; i < CAPACITY; i++) {
                if (confirmedList[i] != null) {
                    if (confirmedList[i].getCustomerName().equalsIgnoreCase(string)
                        ...
                        System.out.println("Passenger with following information has been removed from confirmed list: ");
                        temp.display();
                    }
                }
            }
